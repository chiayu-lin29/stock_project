WARNING:  database "stockdb" has no actual collation version, but a version was recorded
--
...
CREATE TABLE public.market_news (
    ...
    CONSTRAINT chk_market_news_market CHECK (((market)::text = ANY ((ARRAY['TW'::character varying, 'US'::character varying])::text[]))),
    CONSTRAINT chk_market_news_symbol_type CHECK (((symbol_type)::text = ANY ((ARRAY['equity'::character varying, 'index'::character varying, 'etf'::character varying, 'other'::character varying])::text[])))
);
...
SELECT pg_catalog.setval('public.watch_list_id_seq', 1, false);
